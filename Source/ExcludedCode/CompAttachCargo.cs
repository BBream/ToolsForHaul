...
namespace Vehicle
{
    public class CompAttachCargo : CompAttachBase
    {
        ...
        {
            //attachments.First().SpawnSetup();

        }

        public override void CompTick()
        {
            Log.Message("Attachment:" + attachments.Count);
            Log.Message("Type - " + attachments.First().GetType().ToString());
            attachments.First().Tick();
        }
